<template>
  ...
  <div class="relative mt-2 flex flex-col gap-20 px-5">
    ...
    <form @submit.prevent="DeleteJeux">
...
export default {
  ...
  methods: {
    async getJeux(id) {
      ...
      this.refjeux = await getDoc(docRef);
      ...
      if (this.refjeux.exists()) {
        ...
        this.console.log("Jeux inexistant");
      }
      // Obtenir le Storage
    ...
    async DeleteJeux() {
      ...
      await deleteDoc(doc(firestore, "jeux", this.$route.params.id));

      // Suppresson de l'image
      const storage = getStorage();
      // Référence du fichier de la photo
      let docRef = ref(storage, "logojeux/" + this.jeux.photo);
